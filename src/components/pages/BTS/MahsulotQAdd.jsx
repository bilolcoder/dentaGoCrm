  ...
  const uploadImagesToServer = async (imageFiles) => {
    ...
    for (let i = 0; i < imageFiles.length; i++) {
      try {
        ...
        const response = await axios.post(`${BASE_URL}/api/upload/image`, formDataImage, {
          headers: {
        ...
        // Server faqat filename qaytaradi (string)
        const filename = typeof response.data === 'string'
          ? response.data.trim()
          : response.data.filename || `unknown_${Date.now()}_${i}.jpg`;

        // To'liq ma'lumot saqlash uchun (keyinroq foydalanish uchun)
        const imageData = {
          url: `${BASE_URL}/uploads/${filename}`, // frontendda ko'rsatish uchun to'liq URL
          filename: filename, // serverga yuborish uchun faqat nom
        ...
        console.log(`Rasm yuklandi: ${filename}`);

      } catch (error) {
        console.error(`Rasm ${i + 1} yuklanmadi:`, error.response?.data || error.message);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (images.length > 0) {
        ...
        uploadedImagesData = await uploadImagesToServer(images);
        console.log("Yuklangan rasmlar ma'lumotlari:", uploadedImagesData);
      }

      const imageUrls = uploadedImagesData.map(img => img.url);
      ...
      console.log("Yuborilayotgan mahsulot ma'lumotlari:", payload);
      ...
      const successMessage = `✅ Mahsulot muvaffaqiyatli qo'shildi!${
        uploadedImagesData.length > 0
          ? `\n${uploadedImagesData.length} ta rasm yuklandi.`
          : '\nRasmsiz qo\'shildi.'
      }`;
